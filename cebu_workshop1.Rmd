...
Observe results. What patterns in the text can you match using the above five rules? What if you want to match only  

(1) four items with lisinopril, or  
(2) two beta blockers (end with -lol)?  

### Answer

(1) `LlIiSsIiNnOoPpRrIiLl`  
(2) `[A-Za-z]*lol`
...
**More regular expressions, courtesy by [Dave Child](https://www.cheatography.com/davechild/cheat-sheets/regular-expressions/pdf/)**
...
```{r}
f <- "https://raw.githubusercontent.com/ckbjimmy/2017_cebu/master/data/id.txt"

data <- readChar(f, file.info(f)$size)
...
Here we downloaded publicly available 431 clinical notes and reports from [iDASH Clinical Notes and Reports Repository](https://idash-data.ucsd.edu/community/45/), and annotated the documents with the suitable document theme by two clinicians (Weng, 2017). 

```{r}
f <- "https://raw.githubusercontent.com/ckbjimmy/2017_cebu/master/data/idash.txt"

idash <- read.table(f, sep = "\t", header = FALSE, comment.char="", stringsAsFactors = FALSE)
data <- idash$V1
...
The document-term matrix can be an input of different machine learning algorithms. For example, the unsupervised learning algorithm, k-means. However, the plot shows that the performance of clustering is not that good. We will use other language modeling techniques to make it better in the next workshop.  
...
Now we try to play with the [**PhysioNet Deidentified Medical Text**](https://physionet.org/works/DeidentifiedMedicalText/). 

```{r}
f <- "https://raw.githubusercontent.com/ckbjimmy/2017_cebu/master/data/id.txt"

x <- readChar("data/id.txt", file.info("data/id.txt")$size)
x <- gsub("\n\n\\|\\|\\|\\|END_OF_RECORD\n\nSTART_OF_RECORD=[0-9]+\\|\\|\\|\\|[0-9]+\\|\\|\\|\\|\n", " [split] ", x)
x <- gsub("\n\n\\|\\|\\|\\|END_OF_RECORD\n\n", "", x)
x <- strsplit(x, " \\[split\\] ")
x <- x[[1]]
...
1. How many terms/phrases has frequency >= 50?
